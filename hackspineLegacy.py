...
def taskCalendar(inputTask):
    ...
    if days > 98:
        ...
        print("\033[38;5;196mThe task is literally overdue, the only plan I can give is to finish the task ASAP!")
    else:
...
while True:
    if (not setup[2]):
        input("\033[38;5;014mHello, welcome to sPlanner! Press enter to get started with the setup!")
        setup[0] = input("\033[38;5;135mName of project: ")
        try:
        ...
        while not success:
            try:
                ...
                success = False
        setup[2] = True
        print("\033[38;5;014mSetup is complete! Type 'h' for help and to view the commands :)\033[38;5;197m")
    ...
    elif userCommand == "a": # add a task
        taskAdd = newTask()
        tasks.append(taskAdd)
        print("Added task! :)")
    elif userCommand == "v": # print tasklist
        printTasks(tasks)
    elif userCommand == "s": # finds out how much work you should do per day
        workPerDay()
    elif userCommand == "p":
        try:
            progress = int(input("\033[38;5;226mInvalid input. How much progress have you gotten so far on your goals for today so far? Only enter a number: "))
            success = True
        except:
            success = False
        while not success:
